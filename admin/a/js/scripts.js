...
function _get_page(page,ids, divid) {
			...
			success: function (info) {
				...
				if (result == true) {
					for (var i = 0; i < fetch.length; i++) {
						...
						text +=
							'<div class="user-div animated fadeIn" title="View Profile" onclick="_get_form_with_id(' + "'staff_profile'" + ',' + "'" + staff_id + "'" + ')">' +
			...
			success: function (info) {
				...
				if (result == true) {
					for (var i = 0; i < fetch.length; i++) {
						...
						text +=
							'<div class="user-div animated fadeIn" title="View Profile" onclick="_get_form_with_id(' + "'user_profile'" + ',' + "'" + user_id + "'" + ')">' +
							'<div class="pix-div"><img src="' + website_url + '/uploaded_files/staff_pix/' + passport + '"/></div>' +
							'<div class="detail">' +
							'<div class="name-div"><div class="name">' + fullname.toUpperCase() + '</div><hr /><br/></div>' +
...
function _add_and_update_subject(page, subject_id) {
	...
	$('#subject_name, #subject_summary, #subject_url, #seo_keywords, #seo_description ,#status_id').removeClass('complain');

	if (subject_name == '') {
		$('#subject_name').addClass('complain');
		$('#warning-div').html('<div><i class="bi-exclamation-triangle"></i></div> SUBJECT NAME ERROR!<br /><span>Check Subject Name And Try Again</span>').fadeIn(500).delay(3000).fadeOut(100);

	} else if (subject_summary == '') {
		$('#subject_summary').addClass('complain');
		$('#warning-div').html('<div><i class="bi-exclamation-triangle"></i></div> SUBJECT SUMMARY ERROR!<br /><span>Check Summary And Try Again</span>').fadeIn(500).delay(3000).fadeOut(100);

	} else if (subject_url == '') {
		$('#subject_url').addClass('complain');
		$('#warning-div').html('<div><i class="bi-exclamation-triangle"></i></div> SUBJECT URL ERROR!<br /><span>Check Url And Try Again</span>').fadeIn(500).delay(3000).fadeOut(100);

	} else if (seo_keywords == '') {
		$('#seo_keywords').addClass('complain');
		$('#warning-div').html('<div><i class="bi-exclamation-triangle"></i></div> SEO KEYWORDS ERROR!<br /><span>Check Seo Keywords And Try Again</span>').fadeIn(500).delay(3000).fadeOut(100);

	} else if (seo_description == '') {
		$('#seo_description').addClass('complain');
		$('#warning-div').html('<div><i class="bi-exclamation-triangle"></i></div> SEO DESCRPITION ERROR!<br /><span>Check Seo Description And Try Again</span>').fadeIn(500).delay(3000).fadeOut(100);

	} else if (status_id == '') {
		$('#reg_status_id').addClass('complain');
		$('#warning-div').html('<div><i class="bi-exclamation-triangle"></i></div> STATUS ERROR!<br /><span>Check Status And Try Again <span>').fadeIn(500).delay(3000).fadeOut(100);
			...
			$.ajax({
				...
				success: function (info) {
					...
					if (result == true) {
						...
						if (subject_picture != '') {
							_upload_pix(page, message1, message2, subject_picture,'', old_passport);
						} else {
							$('#success-div').html('<div><i class="bi-check"></i></div> ' + message1 + ' <br> ' + message2 + ' ').fadeIn(500).delay(5000).fadeOut(100);
							_alert_close();
...
function _upload_pix(page, message1, message2, subject_picture, exam_logo, old_passport) {
	...
	$.ajax({
		...
		success: function (html) {
			...
			_get_page('exam_category', 'exam_category', 'exam_category', '');	
			}
			_alert_close();
		}
	});
				...
				if (result == true) {
					for (var i = 0; i < fetch.length; i++) {
						...
						if (div_id == 'fetch_subject') {
							...
							text +=
								'<label>' +
								'<input type="checkbox" class="child" id="'+subject_id+'" name="subject_id[]" data-value="' + subject_id + '" />' +
...
function _add_and_update_exam(page,exam_id) {
	...
	var exam_summary = $('#exam_summary').val();
	...
	$('#abbreviation, #exam_name, #exam_summary, #exam_url, #seo_keywords, #seo_description, #subject_id, #status_id').removeClass('complain');

	if (abbreviation == '') {
		$('#abbreviation').addClass('complain');
		$('#warning-div').html('<div><i class="bi-exclamation-triangle"></i></div> ABBREVIATION ERROR!<br /><span>Check And Try Again</span>').fadeIn(500).delay(3000).fadeOut(100);

	} else if (exam_name == '') {
		$('#exam_name').addClass('complain');
		$('#warning-div').html('<div><i class="bi-exclamation-triangle"></i></div> EXAM NAME ERROR!<br /><span>Check Exam Name And Try Again</span>').fadeIn(500).delay(3000).fadeOut(100);

	} else if (exam_summary == '') {
		$('#exam_summary').addClass('complain');
		$('#warning-div').html('<div><i class="bi-exclamation-triangle"></i></div> EXAM SUMMARY ERROR!<br /><span>Check Exam Summary And Try Again</span>').fadeIn(500).delay(3000).fadeOut(100);

	} else if (exam_url == '') {
		$('#exam_url').addClass('complain');
		$('#warning-div').html('<div><i class="bi-exclamation-triangle"></i></div> EXAM URL ERROR!<br /><span>Check Exam URL And Try Again</span>').fadeIn(500).delay(3000).fadeOut(100);
		...
		$('#seo_description').addClass('complain');
		$('#warning-div').html('<div><i class="bi-exclamation-triangle"></i></div> SEO DESCRIPTION ERROR!<br /><span>Check Seo Description And Try Again</span>').fadeIn(500).delay(3000).fadeOut(100);

	 } else if (checked == 0) {
		$('#subject_id').addClass('complain');
		$('#warning-div').html('<div><i class="bi-exclamation-triangle"></i></div> SUBJECT ERROR!<br /><span>Pick at least 1 Subject And Try Again</span>').fadeIn(500).delay(3000).fadeOut(100);
		...
		if (confirm("Confirm!!\n\n Are you sure to PERFORM THIS ACTION?")) {
			...
			form_data.append('exam_summary', exam_summary);
			...
			success: function (info) {
				...
				if (result == true) {
							...
							'<div class="record-content-div animated fadeIn">'+
								'<div class="div-in">'+
									...
									'<div class="text-div">'+
										...
										'<div class="count-div">'+
											'<div class="count-in"><i class="bi-book"></i> TOPICS: <span id="">100</span> &nbsp;|&nbsp; <i class="bi-book"></i> SUB-TOPICS: <span id="">100</span> &nbsp;|&nbsp; <i class="bi-book"></i> STATUS: <span class="ACTIVE" id="">'+ status_name +'</span></div>'+
											'<button class="btn" title="EDIT" onClick="_get_form_with_id(' + "'add_and_update_exam'" + ',' + "'" + exam_id + "'" + ');"><i class="bi-pencil-square"></i> EDIT</button>'+
					...
					text +=
						'<div class="false-notification-div">' +
						'<p> ' + message + ' </p>' +
						'<button class="btn" onclick="_get_form(' + "'add_and_update_exam'" + ')"><i class="bi-plus-square"></i>ADD NEW EXAM</button>' +
						'</div>';
					$('#fetch_exam').html(text);
				}
...
function _fetch_each_exam(exam_id) {
	...
	$.ajax({
		...
		success: function (info) {
			...
			var exam_summary = fetch.exam_summary;
...
function _get_view(exam_passport) {
	var view_exam = '';
	if (exam_passport == '') {
		view_exam = '<img src="' + website_url + '/uploaded_files/exam_pix/default.png" id="exam-pix" alt="profile picture" />'
	} else {
		view_exam = '<img src="' + website_url + '/uploaded_files/exam_pix/' + exam_passport + '" id="exam-pix" alt="profile picture" />'
	}
...
function _get_fetch_exam_subject(exam_id) {
	...
	if ((search_txt.length > 2) || (search_txt == '')) {
		...
		var dataString = 'action=' + action + '&exam_id=' + exam_id + '&status_id=' + status_id + '&search_txt=' + search_txt;
		$.ajax({
			type: "POST",
			url: api,
			data: dataString,
				...
				if (result == true) {
					for (var i = 0; i < fetch.length; i++) {
						...
						text +=
							...
							'<hr></hr>' +
							'<div class="count-div"><i class="bi-book"></i> TOPICS: <span id="">100</span> &nbsp;|&nbsp; <i class="bi-book"></i> SUB-TOPICS: <span id="">100</span> </div>' +
							'<button class="btn btn2" title="EDIT" onClick="_get_page(' + "'topics'" + ',' + "'" + subject_id + "'" + ',' + "'exam'" + ');"><i class="bi-pencil-square"></i> VIEW TOPICS</button>'+
							'</div>' +
							'</div>' +
							'</div>';
